...
<div class="flex h-screen overflow-hidden">
    <div class="flex-1 overflow-y-auto p-6 bg-gray-50">
        <div class="max-w-2xl">
            <div class="mb-4">
                ...
                <p class="text-gray-600 text-sm mt-1">Update medication information</p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full md:w-[500px] bg-white shadow-2xl overflow-y-auto border-l border-gray-200">
        ...
        <form action="{{ route('super-admin.medications.update', $medication->id) }}" method="POST" class="p-4 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="bg-gray-50 rounded-lg p-3 border border-gray-200">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="brand_name" value="{{ old('brand_name', $medication->brand_name) }}" 
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                </div>

                <div class="mb-3">
                    <label class="block text-xs font-medium text-gray-700 mb-1">Category</label>
                    <input type="text" name="category" value="{{ old('category', $medication->category) }}" 
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="route" value="{{ old('route', $medication->route) }}" 
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                ...
                <div class="mb-3">
                    <label class="flex items-center text-sm text-gray-700">
                        <input type="checkbox" name="requires_prescription" value="1" {{ old('requires_prescription', $medication->requires_prescription) ? 'checked' : '' }} 
                ...
                <div class="mb-3">
                    ...
                    <select name="status" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                        <option value="active" {{ old('status', $medication->status) == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $medication->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-3 border border-gray-200">
                ...
                <div class="mb-3">
                    <textarea name="description" rows="3" 
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                        placeholder="Medication description...">{{ old('description', $medication->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="flex gap-2 pt-3 sticky bottom-0 bg-white pb-3 border-t border-gray-200 -mx-4 px-4">
                <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg shadow-lg transition-all text-sm">
                    <i class="fas fa-save mr-1"></i> Update Medication
                </button>
                <a href="{{ route('super-admin.medications.show', $medication->id) }}" class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 rounded-lg text-center transition-all text-sm">
