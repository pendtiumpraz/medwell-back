...
@section('content')
<div x-data="facilityManagement()">
    <!-- Page Header -->
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 font-sora">Facilities Management</h1>
            <p class="text-gray-600 mt-1">Manage all healthcare facilities</p>
        </div>
        <button 
            @click="openCreate()"
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div class="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
            ...
            <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Filter Facilities</h3>
        </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Facility name..." class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary transition-all">
            </div>
            <div>
                <label class="flex items-center gap-2 text-sm font-medium text-gray-700 mb-2">
                ...
                <select name="organization_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary transition-all">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $facility->organization->name ?? 'N/A' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $facility->type ?? 'N/A' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $facility->phone ?? 'N/A' }}</td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-sm">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('super-admin.facilities.show', $facility->id) }}" class="inline-flex items-center justify-center w-8 h-8 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-colors" title="View Details">
                            ...
                            <button @click="openEdit({{ $facility->id }})" class="inline-flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors" title="Edit">
                            ...
                            <form method="POST" action="{{ route('super-admin.facilities.destroy', $facility->id) }}" class="inline" onsubmit="return confirm('Delete?')">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">No facilities found</td></tr>
                @endforelse
            </tbody>
        </table>
        <div class="px-6 py-4 border-t">{{ $facilities->links() }}</div>
    </div>
        ...
        <aside x-show="rightSidebarOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" class="absolute right-0 top-0 h-full w-96 bg-white shadow-2xl overflow-y-auto">
            <div class="h-full flex flex-col">
                <div class="flex items-center justify-between p-6 border-b bg-gradient-to-r from-primary to-primary-dark">
                    <h2 class="text-xl font-bold text-white font-sora" x-text="editMode ? 'Edit Facility' : 'Add Facility'"></h2>
                ...
                <div class="flex-1 overflow-y-auto p-6">
                    <form :action="formAction" method="POST" @submit="handleSubmit">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="organization_id" x-model="formData.organization_id" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary">
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Facility Name *</label>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="type" x-model="formData.type" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary" placeholder="Hospital, Clinic, etc.">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="address" x-model="formData.address" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary"></textarea>
                        </div>
...
function facilityManagement() {
    return {
        ...
        formMethod: 'POST',
        isSubmitting: false,
        formData: { organization_id: '', name: '', type: '', phone: '', address: '', status: 'active' },
        ...
        openCreate() {
            this.editMode = false;
            this.formAction = '{{ route("super-admin.facilities.store") }}';
        ...
        async openEdit(id) {
            this.editMode = true;
            this.editId = id;
            this.formAction = '/super-admin/facilities/' + id;
            this.formMethod = 'PUT';
            this.rightSidebarOpen = true;
            
            try {
                const response = await fetch(`/super-admin/facilities/${id}/json`);
        ...
        resetFormData() {
            this.formData = { organization_id: '', name: '', type: '', phone: '', address: '', status: 'active' };
        },
